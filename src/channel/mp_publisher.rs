...
impl<T: Pod> MpPublisher<T> {
    ...
    /// uncontended case this succeeds immediately — one RMW on the cursor line.
    ///
    ...
    fn advance_cursor(&self, seq: u64) {
        ...
        if seq > 0 {
            ...
            let pred_slot = unsafe { &*self.slots_ptr.add(self.index.slot(seq - 1)) };
    ...
    fn catch_up_cursor(&self, mut seq: u64) {
        ...
        loop {
            ...
            let slot = unsafe { &*self.slots_ptr.add(self.index.slot(next)) };
